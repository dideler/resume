...
{\href{https://rekki.com/}{REKKI}, London, UK}
{
  \begin{itemize} \itemsep -1pt
    \item Early backend engineer focused on building team, product, and platform
    \item Laid foundations for managing release and operational risks
    %     feature flags, logs, metrics, monitoring, alerting,
    %     post mortems, error reporting, release notes,
    %     audit trail for tickets and releases,
    %     run backend on k8s for no-downtime deploys,
    %     high delivery rate for orders via email and sms
    %     (spam detection, sms optimisation, reporting),
    %     management for cloud + secrets + database
    %     error alerting and recovery via chat ops,
    %     secret rotation, tests, data reports and requests,
    ...
    \item Empowered teams by tackling major technical debt across architecture and data
    %     implement new data structures for buyers and suppliers
    %     migrate from couchdb (nosql) to postgres (sql)
    %     move from http and async amqp apis to centralised db w/ polling and locking
    \item Enabled data science efforts by building initial data pipeline and delivery platform
    ...
    %     built ETL (extract, transform, load) data pipeline with redshift to speed up their work
    %     and reduce load on production db - but ended up going with a read replica and stronger instance
    \ifCV{
    \item Built fully documented internal and public APIs for buyers and suppliers
    \item Managed a core subset of our early cloud infrastructure and roles
    \item Organised our first all hands and spoke at several on behalf of the tech team
    }
   \end{itemize}
