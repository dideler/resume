...
\newcommand{\PersonalEntry}[2]{           % Args: Entry name, Entry value
    \noindent\hangindent=0em\hangafter=0  % Indentation
    \parbox{\spacebox}{                   % Box to align text
    \textit{#1}}                          % Entry name (year, label, address, etc.)
    \hspace{1.0em} #2 \par}               % Entry value with spacing

\newcommand{\SkillsEntry}[2]{             % Args: Entry name, Entry value
    \noindent\hangindent=0em\hangafter=0  % Indentation
    \parbox{\spacebox}{                   % Box to align text
    \textit{#1}}                          % Entry name (year, label, address, etc.)
    \hspace{1.0em} #2 \par}               % Entry value with spacing

\newcommand{\AwardsEntry}[2]{             % Args: Entry name, Entry value
    \noindent\hangindent=0em\hangafter=0  % Indentation
    \parbox{\spacebox}{                   % Box to align text
    \textit{#1}}                          % Entry name (year, label, address, etc.)
    \hspace{1.0em} #2 \par}               % Entry value with spacing
...
\newcommand{\WorkEntry}[6]{%
  ...
  % Expansion logic:
  \getenv[\CV]{CV}
  \ifthenelse{\equal{#6}{collapseDetailsOnResume} \AND \not\equal{\CV}{true}}{}{%
    \noindent\hangindent=2em\hangafter=0 \small #4  % Description
    \normalsize \par
    \ifx&#5&\else % Consider removing this arg and making tools an item in description, e.g. `'\item \textbf{Tools:} ...`
      {\small
        \hspace{0.0em}\textbf{Tools:} #5 \par       % Tools (if provided)
        % If you want Tools section aligned with bullet point instead of bullet's text:
        % \noindent\hangindent=0em\hangafter=0 \textbf{Tools:} #5 \par
      }
    \fi
  }
  \sepspace
}
% E.g.: \WorkEntry{Title}{Dates}{Company}{Description}{[Tools]}[{collapseDetailsOnResume}]
...
\WorkEntry{Contract Sofware Engineer (Thimble Consulting)}{Mar 2025 -- Jul 2025}
{\href{https://moderntreasury.com}{Modern Treasury}, San Francisco, US (Remote from London)}
{
  \begin{itemize} \itemsep -1pt
    \item Collaborated with a small team and stakeholders to design and implement a data-intensive accounting product under strict operational, technical, and regulatory constraints.
    \item Enabled users to create, view, manage, download, and export journals with complex transaction selection, automated generation, grouping, and validation logic; built interactive tables, configurable settings, and user-facing notifications to ensure a responsive, intuitive experience.
  \end{itemize}
}
{Ruby, TypeScript, React, PostgreSQL, ParadeDB, GraphQL}
...
\WorkEntry{Contract Software Engineer (Thimble Consulting)}{Jun 2020 -- Feb 2022}
{\href{https://glossier.com}{Glossier}, New York, US (Remote from London)}
{
  \begin{itemize} \itemsep -1pt
...
{Ruby, TypeScript, PostgreSQL, Fivetran, Snowflake, Solidus, AWS, Docker, DataDog}
...
%     %   - move backend to k8s for no downtime deploys, w/ auto-rollbacks (from docker and shell scripts)
...
%     \item Introduced core concepts of buyer, branch, and supplier into the codebase and data schemas
...
%    \end{itemize}
% }
% {Elixir, Ruby, Python, Go, JS, RabbitMQ, PostgreSQL, AWS, Docker, K8s, Grafana}
...
{
  \begin{itemize} \itemsep -1pt
    \item Joined in Series A to scale and stabilize the platform amid rapid growth; introduced engineering rituals (standups, ticketing, monitoring), throttled alerts for efficient firefighting, and built foundational tools like chat bots, growth automation, and API throttling to boost operational resilience
    \item Led backend and infrastructure teams through Series B as tech lead, reporting to Founder & CTO; defined roadmaps, KPIs, SLAs, and on-call practices; managed AWS/K8s infrastructure for zero-downtime deploys, auto-rollbacks, and incident response; drove recruitment, onboarding, and a data-driven mindshift to minimize risks
    \item Pioneered data engineering by architecting the initial ETL pipeline, Redshift warehouse, and SageMaker-based delivery platform; enabled reproducible model serving via Docker, APIs, and version control, reducing prod DB load with read replicas and empowering analytics-driven decisions
    \item Eliminated architectural and data debt through migrations: CouchDB to PostgreSQL for joinable data, Elixir/RabbitMQ to streamlined Go for order flows, and OVH to AWS for improved performance and reduced latency; introduced core concepts of buyer, branch, and supplier into the codebase
    \item Delivered critical backend capabilities across product, growth, and operations: real-time WebSocket/HTTP APIs, supplier HTTP/FTP integrations, invoice uploading, automated GDPR handling, spam detection, SMS/email optimization, targeted push notifications, and a localisation pipeline to enable rapid international expansion
  \end{itemize}
}
{Elixir, Ruby, Python, Go, JS, RabbitMQ, PostgreSQL, AWS, Docker, K8s, Grafana}
...
{JavaScript, React, GraphQL, Elixir, Phoenix, TeamCity, Trello, LaunchDarkly}
...
{\href{https://www.fundingcircle.com/}{Funding Circle}, London, UK}
{
  \begin{itemize} \itemsep -1pt
    ...
    % \item \textbf{Tools:} Ruby, Rails, PostgreSQL, GitHub, Capistrano, Jenkins, Docker, JIRA, AWS, RabbitMQ
  \end{itemize}
}
{Ruby, Rails, PostgreSQL, GitHub, Capistrano, Jenkins, Docker, JIRA, AWS, RabbitMQ}
...
\WorkEntry{Consulting Software Engineer}{Sep 2014 -- Feb 2015}
{\href{https://www.landlordwebsolutions.com/}{Landlord Web Solutions}, Ontario, Canada (Remote from London)}
{
  \begin{itemize} \itemsep -1pt
    \item Lead developer on a project for analytics and metrics in the apartment rental industry % Lead tracking
  \end{itemize}
}
{Ruby, Rails, RSpec, PostgreSQL, Git, GitHub, Heroku, Mailgun, Python}
...
{Ruby, Rails, MySQL, CoffeeScript, Bootstrap, Git, GitHub, Heroku, Sendgrid}
{collapseDetailsOnResume}
...
{Microsoft Sharepoint, JavaScript, jQuery, HTML, CSS, UML}
{collapseDetailsOnResume}
...
{Linux shell scripting (bash), Ruby, Watir, Selenium, Trac, Subversion (SVN)}
{collapseDetailsOnResume}
...
\ifCV{
  ...
  \AwardsEntry{2016}{\href{https://www.fca.org.uk/events/techsprints/consumer-access-techsprint}{Funding Circle won Best Tech award at FCA's inaugarual TechSprint}}
  \AwardsEntry{2012}{\href{https://dideler.github.io/notes/my-history-of-competitive-coding-acm-icpc}{Assistant coach for ACM International Collegiate Programming Contest}}
  \AwardsEntry{2011--2012}{Judge for the Niagara Region Secondary Schools Programming Contest}
  \AwardsEntry{2011}{\href{https://github.com/dideler/gps-tower-defense}{Brock Team placed top 25 at the nationwide Great Canadian Appathon}}
  \AwardsEntry{2008-2011}{\href{https://dideler.github.io/notes/my-history-of-competitive-coding-acm-icpc}{Contestant at ACM International Collegiate Programming Contest}}
}
